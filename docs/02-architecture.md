...
- **No-code/low-code в центре оркестрации** (n8n) — как требует вакансия, плюс тонкий слой
  собственных сервисов на Go (Go 1.26) там, где no-code неудобен (LLM-логика, RAG, нормализация данных).
...
|-----------|------------|--------------------------|------|
| **Channel Gateway** | Приём/отправка сообщений мессенджеров, нормализация в единый формат, верификация вебхуков | Go (net/http) (или встроенные узлы n8n) | E3 |
| **n8n (оркестрация)** | Сценарии воронки, маршрутизация, эскалации, расписания, ретраи | n8n (self-hosted, Docker) | E4 |
| **LLM Gateway** | Единая точка вызова LLM, управление промптами/версиями, выбор модели, логирование токенов | Go | E1 |
| **RAG Service** | Индексация базы знаний, поиск релевантных фрагментов, сборка контекста | Go + Vector DB | E2 |
...
- **Бэкенд-сервисы:** Go 1.26 (стандартная библиотека `net/http`).
